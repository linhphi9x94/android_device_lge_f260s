...
camera_module_t HAL_MODULE_INFO_SYM = {
    common: {
         ...
         name: "Vu2 Camera Wrapper",
         author: "Windxixi build",
...
char * camera_fixup_setparams(int id, const char * settings)
{
    ...
    const char *sceneMode = "auto";
     const char *previewColorMode;
     const char *pictureColorMode;
    android::CameraParameters params;
    params.unflatten(android::String8(settings));

    ALOGV("%s: original parameters:", __func__);
    //params.dump();
	previewColorMode = params.getPreviewFormat();
	pictureColorMode = params.getPictureFormat();
	ALOGV("%s:original parameters:previewColorMode=%s\n",__func__,previewColorMode);
	ALOGV("%s:original parameters:pictureColorMode =%s\n",__func__,pictureColorMode );
	params.setPreviewFormat("nv12");
